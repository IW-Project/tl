...
void *tlScratchPadInit()
{
  ...
  tlScratchPadPtr = (u8 *)tlMemAlloc( 0x4000, 16, 0 );
  return tlScratchPadPtr;
}
...
bool tlReadFile( const char *FileName, tlFileBuf *File, unsigned int Align, unsigned int Flags )
{
  ...
  HANDLE hFile =
      CreateFileA( filePath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL );
  ...
  void *pBuffer = tlMemAlloc( dwFileSize, Align, Flags );
  File->Buf = (u8 *)pBuffer;
...
int tlCeilDiv( int x, int y )
{
  return ( y + x - 1 ) / y;
}

int tlCountOnes( int v )
{
  int c = 0;
